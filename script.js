...
// Setup event listeners for circles
function setupCircles() {
  ...
  function clickCircle(clickedCircle) {
	var clickedColor = clickedCircle.style.backgroundColor;
	if (clickedColor === pickedColor) {
	  ...
	  clickedCircle.style.backgroundColor = defaultColour;
	  resultMessage.textContent = "Try again";
	}
  }
  
  // Change all circles to the correct color upon winning
...
function reset() {
	colours = genRandomColours(numCircles); // Generate all new colors
	pickedColor = chooseColor(); // Pick a new random color from the array
	colourToGuess.textContent = pickedColor; // Change colorToGuess to match picked color
	for (var i = 0; i < circles.length; i++) {
